...
const downloadExcel = async () => {
  ...
  const totalSum =
    (Array.isArray(assistances) ? assistances.reduce((acc, i) => acc + Number(i.total || 0), 0) : 0) +
    (Array.isArray(confirmedAssistances) ? confirmedAssistances.reduce((acc, i) => acc + Number(i.total || 0), 0) : 0) +
    (Array.isArray(assistancesByPrograma) ? assistancesByPrograma.reduce((acc, i) => acc + Number(i.cantidad_registros || 0), 0) : 0) +
    (Array.isArray(confirmedAssistancesByPrograma)
      ? confirmedAssistancesByPrograma.reduce((acc, i) => acc + Number(i.cantidad_registros || 0), 0)
      : 0);
    ...
    <ImageBackground source={require('../../assets/images/banner.jpg')} style={styles.background}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <ReportTable
          ...
          renderItem={renderRow('programaInteres', 'total')} // ← Cambiar esto si no es cantidad_registros
        ...
        {/* <View style={styles.totalContainer}>
          <Text style={styles.totalText}>Suma total de registros: {totalSum}</Text>
        </View> */}
      </ScrollView>
    </ImageBackground>
  );
